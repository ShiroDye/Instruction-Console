...
root = Tk()
root.title("Instruction Sequence Console Version 3.2.1 By ShiroDye")
root.geometry("700x620+500+150") # Slightly increased height for better layout
...
def run_task():
    ...
    try:
        ...
        while loop_idx < count:
            if stop_flag: break
            status_text = f"Progress: Loop {loop_idx+1}" + (" (Inf)" if count == float('inf') else f" / {count}")
        ...
        force_reset_keys()
        stop_flag = False
        btn.config(text="START EXECUTION [Ctrl+Shift+F12]", bg='#5c6bc0', command=start_task)
...
def start_task():
    ...
    if show_countdown_flag.get():
        ...
        time.sleep(1) # Small delay to prevent key sticking from the click/hotkey
        threading.Thread(target=run_task, daemon=True).start()

def show_countdown():
    countdown_win = Toplevel(root)
    countdown_win.title("Countdown")
    countdown_win.geometry("350x220+700+150")
    countdown_win.attributes("-topmost", True)
    countdown_win.resizable(False, False)
    countdown_win.configure(bg='#e53935')
    
    msg = "⚠ Execution starting, switch to target window!\nℹ You can disable this countdown in the Debug Menu."
    hint_label = Label(countdown_win, text=msg, fg='white', bg='#e53935', 
                      font=('Arial', 10, 'bold'), wraplength=320)
...
def toggle_record():
    ...
    rec_win.geometry("220x90+20+20")
    rec_win.overrideredirect(True)
    rec_win.configure(bg="#c0392b")
    Label(rec_win, text="Recording... (System Keys Blocked)", fg="white", bg="#c0392b").pack(pady=10)
    ...
    Button(rec_win, text="Finish & Save", command=stop_rec, bg="white", relief=FLAT).pack()
...
def setup_hotkey_listener():
    ...
    def on_hotkey_press(key):
        try:
            ...
            ctrl = keys_pressed.get("ctrl_l", False) or keys_pressed.get("ctrl", False) or keys_pressed.get("ctrl_r", False)
            shift = keys_pressed.get("shift_l", False) or keys_pressed.get("shift", False) or keys_pressed.get("shift_r", False)
            f12 = keys_pressed.get("f12", False)
            
            if ctrl and shift and f12:
...
key_list_menu.add_cascade(label="FN Keys", menu=fn_menu)

debug_menu.add_command(label="Enable Recording Mode", command=toggle_record)
debug_menu.add_command(label="Import File", command=import_txt)
debug_menu.add_separator()
debug_menu.add_checkbutton(label="Enable Countdown Window", variable=show_countdown_flag)
debug_menu.add_separator()
debug_menu.add_cascade(label="Quick Insert Key", menu=key_list_menu)
debug_menu.add_cascade(label="Adjust Speed", menu=speed_menu)
main_menu.add_cascade(label="Debug Menu", menu=debug_menu)
root.config(menu=main_menu)

f_top = Frame(root, bg='#3b3e4b', height=40); f_top.pack(fill=X)
Label(f_top, text="Instruction Sequence Console Version 3.2.1", fg='white', bg='#3b3e4b', font=('Arial', 11, 'bold')).pack(pady=5)
...
Label(f_body, text="Loop Count ('inf' for infinite):", bg='#f4f4f9', fg='#555').pack(anchor=W)
...
btn = Button(f_body, text="START EXECUTION [Ctrl+Shift+F12]", bg='#5c6bc0', fg='white', font=('Arial', 12, 'bold'), relief=FLAT, command=start_task)
