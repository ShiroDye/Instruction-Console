...
root = Tk()
root.title("指令序列控制台 Version 3.1  By ShiroDye")
...
def run_task():
    ...
    try:
                    ...
                    if inner.upper().startswith('BLANK:'):
                        ...
                        if objs:
                            try:
                                for o in objs:
                                    k_control.press(o)
                                    time.sleep(0.05) # 按下后的识别延迟
                                time.sleep(0.1)     # 核心：所有键按住后的保持时间
                            finally:
        ...
        force_reset_keys()
        stop_flag = False
        btn.config(text="开始执行", bg='#5c6bc0', command=start_task)
...
Label(f_top, text="指令序列控制台 Version 3.1", fg='white', bg='#3b3e4b', font=('微软雅黑', 11)).pack(pady=5)
...
btn = Button(f_body, text="开始执行", bg='#5c6bc0', fg='white', font=('微软雅黑', 12, 'bold'), relief=FLAT, command=start_task)
