...
root = Tk()
root.title("指令序列控制台 Version 3.2.0 By ShiroDye")
...
def run_task():
    ...
    try:
        ...
        btn.config(text="停止执行", bg='#e53935', command=stop_task)
        
        loop_idx = 0
        while loop_idx < count:
            if stop_flag: break
            counter_label.config(text=f"进度: 第 {loop_idx+1} 轮" + (" (无限)" if count == float('inf') else f" / {count}"))
        ...
        force_reset_keys()
        stop_flag = False
        btn.config(text="开始执行 [Ctrl+Shift+F12]", bg='#5c6bc0', command=start_task)
...
def show_countdown():
    ...
    hint_label = Label(countdown_win, text="⚠ 即将开始执行，请尽快切入目标窗口!\nℹ若不喜欢我，可以在左上角 Debug Menu 里关闭倒计时功能。", 
                      fg='white', bg='#e53935', font=('微软雅黑', 10, 'bold'), wraplength=320)
...
def toggle_record():
    ...
    Label(rec_win, text="录制中 (系统键已拦截)", fg="white", bg="#c0392b").pack(pady=10)
    ...
    Button(rec_win, text="结束并保存", command=stop_rec, bg="white", relief=FLAT).pack()
...
for l, v in [("极速 (0.1s)", 0.1), ("标准 (0.5s)", 0.5), ("慢速 (1.0s)", 1.0)]:
...
key_list_menu.add_cascade(label="FN", menu=fn_menu)

debug_menu.add_command(label="开启录制模式", command=toggle_record)
debug_menu.add_command(label="导入文件", command=import_txt)
debug_menu.add_separator()
debug_menu.add_checkbutton(label="启用倒计时", variable=show_countdown_flag)
debug_menu.add_separator()
debug_menu.add_cascade(label="快速插入", menu=key_list_menu)
debug_menu.add_cascade(label="调节速度", menu=speed_menu)
main_menu.add_cascade(label="Debug Menu", menu=debug_menu)
root.config(menu=main_menu)

f_top = Frame(root, bg='#3b3e4b', height=40); f_top.pack(fill=X)
Label(f_top, text="指令序列控制台 Version 3.2.0", fg='white', bg='#3b3e4b', font=('微软雅黑', 11)).pack(pady=5)
...
Label(f_body, text="循环次数 (inf 为无限):", bg='#f4f4f9', fg='#555').pack(anchor=W)
entry2 = Entry(f_body, font=('Consolas', 12), bd=1, relief=SOLID)
entry2.insert(0, "1"); entry2.pack(fill=X, pady=(5, 10), ipady=5)
counter_label = Label(f_body, text="就绪", bg='#f4f4f9', fg='#5c6bc0', font=('微软雅黑', 10, 'bold'))
counter_label.pack(pady=2)
btn = Button(f_body, text="开始执行 [Ctrl+Shift+F12]", bg='#5c6bc0', fg='white', font=('微软雅黑', 12, 'bold'), relief=FLAT, command=start_task)
